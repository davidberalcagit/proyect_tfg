        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-[#284961] text-white">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-center text-xs font-medium uppercase tracking-wider w-24">Modelos</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    <td class="px-6 py-4">
                        <input type="text" wire:model="newBrandName" wire:keydown.enter="store" class="w-full border-gray-300 rounded-lg shadow-sm" placeholder="Nueva marca...">
                        @error('newBrandName') <span class="text-red-500 text-xs block mt-1">{{ $message }}</span> @enderror
                    </td>
                    ...
                    <tr wire:key="brand-{{ $brand->id }}" class="hover:bg-gray-50 transition duration-150">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            @if($editingBrandId === $brand->id)
                                <input type="text" wire:model="editingBrandName" wire:keydown.enter="update" class="w-full border-gray-300 rounded-lg shadow-sm">
                                @error('editingBrandName') <span class="text-red-500 text-xs block mt-1">{{ $message }}</span> @enderror
                            @else
                                {{ $brand->nombre }}
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            ...
                            @else
                                <button wire:click="edit({{ $brand->id }})" class="text-[#284961] hover:text-[#1c3344] font-bold">Editar</button>
                                <button wire:click="delete({{ $brand->id }})" wire:confirm="¿Seguro que quieres eliminar esta marca?" class="ml-3 text-red-600 hover:text-red-800 font-bold">Borrar</button>
                            @endif
                        </td>
                    </tr>
                @endforeach

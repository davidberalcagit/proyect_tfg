                    ...
                    <tr wire:key="color-{{ $color->id }}" class="hover:bg-gray-50 transition duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-3">
                                <button wire:click="edit({{ $color->id }})" class="text-[#284961] hover:text-[#1c3344] font-bold transition">
                                    Editar
                                </button>
                                <button wire:click="delete({{ $color->id }})"
                                        onclick="confirm('¿Seguro que quieres borrar este color?') || event.stopImmediatePropagation()"
    ...
    @if($isModalOpen)
        <div class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center backdrop-blur-sm">
            ...
            <div class="bg-white rounded-xl overflow-hidden shadow-2xl transform transition-all sm:max-w-lg sm:w-full p-6 relative z-10 border border-gray-200">
                <h3 class="text-xl font-bold text-[#284961] mb-4">{{ $color_id ? 'Editar' : 'Crear' }} Color</h3>

                    ...
                    <tr wire:key="gear-{{ $gear->id }}" class="hover:bg-gray-50 transition duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-3">
                                <button wire:click="edit({{ $gear->id }})" class="text-[#284961] hover:text-[#1c3344] font-bold transition">
                                    Editar
                                </button>
                                <button wire:click="delete({{ $gear->id }})"
                                        onclick="confirm('¿Seguro que quieres borrar este tipo?') || event.stopImmediatePropagation()"
    ...
    @if($isModalOpen)
        <div class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center backdrop-blur-sm">
            ...
            <div class="bg-white rounded-xl overflow-hidden shadow-2xl transform transition-all sm:max-w-lg sm:w-full p-6 relative z-10 border border-gray-200">
                <h3 class="text-xl font-bold text-[#284961] mb-4">{{ $gear_id ? 'Editar' : 'Crear' }} Caja de Cambios</h3>

                <div class="mb-6">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Tipo</label>
                    <input type="text" wire:model="tipo" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#B35F12] focus:ring focus:ring-[#B35F12] focus:ring-opacity-50 py-2 px-3" placeholder="Ej: Manual">

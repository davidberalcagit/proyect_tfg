<div class="p-4 sm:p-6 bg-white rounded-lg shadow-md">
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <h2 class="text-xl font-bold text-[#284961]">Gestión de Cajas de Cambio</h2>
    </div>
    ...
    <div class="overflow-x-auto bg-white rounded-lg shadow border border-gray-200">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    <td class="px-6 py-4">
                        <input type="text" wire:model="newGearType" wire:keydown.enter="store" class="w-full border-gray-300 rounded-lg shadow-sm" placeholder="Nuevo tipo...">
                        @error('newGearType') <span class="text-red-500 text-xs block mt-1">{{ $message }}</span> @enderror
                    </td>
                ...
                @foreach($gears as $gear)
                    <tr wire:key="gear-{{ $gear->id }}" class="hover:bg-gray-50 transition duration-150">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            @if($editingGearId === $gear->id)
                                <input type="text" wire:model="editingGearType" wire:keydown.enter="update" class="w-full border-gray-300 rounded-lg shadow-sm">
                                @error('editingGearType') <span class="text-red-500 text-xs block mt-1">{{ $message }}</span> @enderror
                            @else
                                {{ $gear->tipo }}
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            ...
                            @else
                                <button wire:click="edit({{ $gear->id }})" class="text-[#284961] hover:text-[#1c3344] font-bold">Editar</button>
                                <button wire:click="delete({{ $gear->id }})" wire:confirm="¿Seguro que quieres eliminar este tipo?" class="ml-3 text-red-600 hover:text-red-800 font-bold">Borrar</button>
                            @endif
                        </td>
                    </tr>
                @endforeach

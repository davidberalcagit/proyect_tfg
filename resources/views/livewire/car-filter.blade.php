                    ...
                    <div class="relative mb-2 sm:mb-4 w-full aspect-square overflow-hidden rounded bg-gray-100">
                        @if($car->image)
                            <img src="{{ Str::startsWith($car->image, 'http') ? $car->image : Storage::url($car->image) }}"
                                 ...
                                 onerror="this.style.display='none'; document.getElementById('fallback-{{ $car->id }}').classList.remove('hidden');">

                            <div id="fallback-{{ $car->id }}" class="hidden bg-gray-200 h-full w-full flex items-center justify-center text-gray-400 absolute inset-0">

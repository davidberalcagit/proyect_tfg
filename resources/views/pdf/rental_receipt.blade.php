...
<body>
    <div class="container">
        ...
        <table>
            ...
            <tbody>
                <tr>
                    <td>{{ __('Rental Cost') }} ({{ $rental->duration_days }} {{ __('days') }} x {{ number_format($rental->car->precio, 2) }} €)</td>
                    <td style="text-align: right;">{{ number_format($rental->precio_total, 2) }} €</td>
                </tr>
                <tr>
                    <td>{{ __('Platform Service Fee') }} (5%)</td>
                    <td style="text-align: right;">{{ number_format($rental->service_fee, 2) }} €</td>
                </tr>
                <tr>
                    <td>{{ __('VAT') }} (21% {{ __('on Service Fee') }})</td>
                    <td style="text-align: right;">{{ number_format($rental->tax, 2) }} €</td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="totals">
            <table>
                <tr>
                    <th>{{ __('Subtotal') }}</th>
                    <td>{{ number_format($rental->precio_total + $rental->service_fee, 2) }} €</td>
                </tr>
                <tr>
                    <th>{{ __('TOTAL PAID') }}</th>
                    <td class="grand-total">{{ number_format($rental->grand_total, 2) }} €</td>
                </tr>
            </table>
        </div>
        <div class="clearfix"></div>
